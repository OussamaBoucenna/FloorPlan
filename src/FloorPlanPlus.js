...
// Fonction pour importer et initialiser le plan d'étage à partir d'un GeoJSON
const initializeFloorPlanFromGeoJSON = (geoJSONData) => {
    ...
    geoJSONData.features.forEach(feature => {
      ...
      if (feature.properties.type === "ImageBoundary") {
        ...
        // };
        console.log("ImageBoundary found:  ----> nothing to do", feature.properties);
        // Optionnel: Ajuster le canvas à la taille de l'image
        // setCanvasSize({ width: imageInfo.width, height: imageInfo.height });
      }
      ...
      // Vous pouvez ajouter la détection d'autres types si nécessaire
    });
    
    return { walls: newWalls, rooms: newRooms, doors: newDoors, imageInfo };
  };
...
//         });
//     }); 
//     // Ajouter ce code à la fin de createGrid avant le return
// console.log("Grille créée - Cellules non traversables:", 
//     grid.flat().filter(cell => !cell.walkable).length);
//   console.log("Cellules de mur:", 
//     grid.flat().filter(cell => cell.isWall).length);
//   console.log("Cellules de buffer:", 
//     grid.flat().filter(cell => cell.isBuffer).length);
//   console.log("Cellules de porte:", 
//     grid.flat().filter(cell => cell.isDoor).length);
